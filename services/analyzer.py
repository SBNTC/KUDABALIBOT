...
## summary: короткий заголовок ДО 60 символов, по-русски. Обязательно укажи название и место если есть.

## event_date: YYYY-MM-DD. ОБЯЗАТЕЛЬНО извлеки дату:
- "15 апреля" при posted=2026-04-10 → "2026-04-15"
- "в субботу" при posted=2026-04-10 (четверг) → "2026-04-12"
- "каждую среду" → ближайшая среда от posted
- Диапазон "15-19 апреля" → первый день "2026-04-15"
- Если дата НЕ указана и невозможно определить → null
Текущий год: CURRENT_YEAR_PLACEHOLDER

## ВАЖНО: ты ОБЯЗАН вернуть результат для КАЖДОГО id из входных данных. Не пропускай.

## ОТВЕТ (строго JSON-массив, ничего больше):
[{"id": 123, "category": "Free", "summary": "Йога в Убуде, 18:00", "event_date": "2026-04-15"}]
    ...
    async with AsyncSessionMaker() as session:
        ...
        for i in range(0, total, BATCH_SIZE):
            ...
            try:
                ...
                prompt = prompt.replace("CURRENT_YEAR_PLACEHOLDER", str(date.today().year))
                raw = await call_deepseek(prompt)

                match = re.search(r'\[.*\]', raw.replace('\n', ' '), re.DOTALL)
                if not match:
                    logging.warning("No JSON in response, marking chunk as Unknown")
                    for e in chunk:
                        await session.execute(
                            update(ScrapedEvent)
                            .where(ScrapedEvent.id == e.id)
                            .values(status=target_status, category="Unknown")
                        )
                        total_processed += 1
                    await session.commit()
                    continue
                ...
                for e in chunk:
                    ...
                    if res and res.get("category") == "Spam":
                        await session.execute(
                            update(ScrapedEvent)
                            .where(ScrapedEvent.id == e.id)
                        ...
                        # AI не вернул результат — отправляем на ручную модерацию
                        await session.execute(
                            update(ScrapedEvent)
                            .where(ScrapedEvent.id == e.id)
                            .values(status=target_status, category="Unknown")
